...
/**
 * Exam details. The questions are built in the Exam Blocks editor
 * (admin/exams/editor.php). Status is edit-only: a new exam starts inactive.
 *
 ...
 */
$showStatus = array_key_exists('status', $values);
?>
<form method="post" action="<?= e($formAction) ?>" novalidate>
    ...
    <div class="row g-3 mb-3">
        <div class="col-sm">
            <label for="pass_percentage" class="form-label required">Pass Percentage</label>
            ...
            <div class="col-sm">
                ...
                <select class="form-select <?= error_for($errors, 'status') ? 'is-invalid' : '' ?>" id="status" name="status" required>
                    <?php foreach (['active' => 'Active', 'inactive' => 'Inactive'] as $value => $label): ?>
                        <option value="<?= $value ?>" <?= $values['status'] === $value ? 'selected' : '' ?>><?= $label ?></option>
                    <?php endforeach; ?>
                </select>
                <?php if ($error = error_for($errors, 'status')): ?>
                    <div class="invalid-feedback"><?= e($error) ?></div>
                <?php else: ?>
                    <div class="form-text">Only active exams can be linked to an induction.</div>
                <?php endif; ?>
            </div>
        <?php endif; ?>
    </div>
